...
parseUInt = Parser f
    where
        ...
        f x = runParser ((\ y -> read y :: Int) <$>
            (parseSome (parseAnyChar ['0' .. '9']))) x -- <$> is the infix version of fmap
    -- where
...
getEscapedChar :: Char -> String -> Maybe (Char, String)
getEscapedChar '\\' zs = Just ('\\', zs)
getEscapedChar '\"' zs = Just ('\"', zs)
getEscapedChar '\'' zs = Just ('\'', zs)
getEscapedChar 'n' zs = Just ('\n', zs)
getEscapedChar 't' zs = Just ('\t', zs)
getEscapedChar 'b' zs = Just ('\b', zs)
getEscapedChar 'r' zs = Just ('\r', zs)
getEscapedChar 'f' zs = Just ('\f', zs)
getEscapedChar 'v' zs = Just ('\v', zs)
getEscapedChar _ _ = Nothing
...
parseEscapedChar = Parser f
    where
        f x = do
            ...
            case runParser (parseAnyChar ['\\', '\"', '\'', 'n', 't',
                'b', 'r', 'f', 'v']) ys of
                Just (z,zs) -> getEscapedChar z zs
...
parseList :: Parser a -> Parser b -> Parser c -> Parser d ->
    Parser e -> Parser [e]
parseList open sep close ignore token = Parser f
    where
        ...
        f x = do
            ...
            (y, ys) <- runParser (parseMany (parseAndWith (\ _ y -> y)
                (parseMany (parseOrBoth sep ignore)) token)) xs
