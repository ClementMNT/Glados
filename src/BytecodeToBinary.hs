...
int32_ToBytes x = map fromIntegral [x .&. 0xFF, (x `shiftR` 8) .&. 0xFF,
    (x `shiftR` 16) .&. 0xFF, (x `shiftR` 24) .&. 0xFF]
...
dataTypeToByte UnknownType =
        trace "ERROR ERROR /!\\: dataTypeToByte: UnknownType" 0x00
...
remplaceJumpRef [] _ _ = []
remplaceJumpRef ((JumpIfTrueBefore x) : xs) jumpId pos
    | jumpId == x = (JumpIfTrue pos : remplaceJumpRef xs jumpId pos)
remplaceJumpRef ((JumpIfFalseBefore x) : xs) jumpId pos
    | jumpId == x = (JumpIfFalse pos : remplaceJumpRef xs jumpId pos)
remplaceJumpRef ((JumpBefore x) : xs) jumpId pos
    | jumpId == x = (Jump pos : remplaceJumpRef xs jumpId pos)
...
findJumpRef ((JumpRef x) : xs) jumpId pos =
    if jumpId == x then
        pos
    else findJumpRef xs jumpId (pos + getLengthOfOperation (JumpRef x))
findJumpRef (x:xs) jumpId pos =
    findJumpRef xs jumpId (pos + getLengthOfOperation x)


--                 bytecode  -> jumpId -> nmb_jmp
remplaceAllJump :: [Bytecode] -> Int -> Int -> [Bytecode]
remplaceAllJump bytecode jumpId nmb_jmp | jumpId > nmb_jmp =
    bytecode
remplaceAllJump bytecode jumpId nmb_jmp =
    remplaceAllJump (remplaceJumpRef bytecode jumpId
    (findJumpRef bytecode jumpId sizeOfHeader)) (jumpId + 1) nmb_jmp
...
remplaceFunEntryPoint ((CallUserFun x) : xs) (pos, name) | x == name =
    (JumpNewScope pos : remplaceFunEntryPoint xs (pos, name))
remplaceFunEntryPoint (x:xs) fun_data =
    x : remplaceFunEntryPoint xs fun_data


--                 bytecode  -> (pos, name) ->      bytecode
remplaceAllFun :: [Bytecode] -> [(Int, String)] -> [Bytecode]
remplaceAllFun bytecode [] = bytecode
remplaceAllFun bytecode (x : xs) =
    (remplaceAllFun (remplaceFunEntryPoint bytecode x) xs)
...
dispAllBytecode (x:xs) p = (dispAllBytecode xs (p + getLengthOfOperation x))
...
toHexaInstruction (LoadConst x t) =
    (0x01 : int32_ToBytes x ++ [dataTypeToByte t])
toHexaInstruction (LoadVarBefore _ _) =
    error "Error: toHexaInstruction: LoadVarBefore" -- it's a reference, it's removed from the bytecode
toHexaInstruction (StoreVarBefore _ _) =
    error "Error: toHexaInstruction: StoreVarBefore" -- it's a reference, it's removed from the bytecode
toHexaInstruction (LoadVar x t) =
    (0x02 : int32_ToBytes x ++ [dataTypeToByte t])
toHexaInstruction (StoreVar x t) =
    (0x03 : int32_ToBytes x ++ [dataTypeToByte t])
...
toHexaInstruction (JumpRef _) =
    error "Error: toHexaInstruction: JumpRef" -- it's a reference, it's removed from the bytecode
...
toHexaInstruction (FunEntryPoint _ _) =
    error "Error: toHexaInstruction: FunEntryPoint" -- it's a reference, it's removed from the bytecode
toHexaInstruction (CallUserFun _) =
    error "Error: toHexaInstruction: CallUserFun" -- it's a reference, it's removed from the bytecode
toHexaInstruction LoadPC =          [0x0F]
toHexaInstruction Index =           [0x10]
toHexaInstruction SaveAt =          [0x11]
toHexaInstruction x =
    error ("Error: toHexaInstruction: Unknown instruction: " ++ show x)

-- * ------------------------------------------ HEADER -------------------------------------------- * --

getHeader :: [Word8]
getHeader = [0x7a, 0x69, 0x7a, 0x69] ++
    (toHexaString "This is the comment section\0")
...
getVariables [] i = ([], i)
getVariables (StoreVarBefore x t : xs) i | differentUnknowType t =
    let (vars, _) = getVariables xs (i + 1) in ((x, t, i) : vars, i)
getVariables (FunEntryPoint _ _ : _) i = ([], i) -- we find another scope, so we stop
getVariables (_:xs) i = getVariables xs i


getScopesVariables :: [Bytecode] -> Int ->
    ([(String, DataType, [(String, DataType, Int)])], Int)
getScopesVariables [] id_local = ([], id_local)
getScopesVariables (FunEntryPoint x t : xs) id_local =
    let (vars, id2) = getVariables xs id_local
        (scopes, id3) = getScopesVariables xs id2
    in ((x, t, vars) : scopes, id3)
getScopesVariables (_:xs) id_local =
    let (scopes, id2) = getScopesVariables xs id_local in (scopes, id2)
...
lookVarInFunction [] _ = []
lookVarInFunction (LoadVarBefore x _ : xs) v =
    (LoadVar (findId v x) (remplaceType v x)) : lookVarInFunction xs v
...
findWhichFunction :: String ->
    [(String, DataType, [(String, DataType, Int)])] ->
    [(String, DataType, Int)]
...
fillTypesInFunctions :: [Bytecode] ->
    [(String, DataType, [(String, DataType, Int)])] -> [Bytecode]
fillTypesInFunctions [] _ = []
fillTypesInFunctions (FunEntryPoint x t : xs) scopes =
    (FunEntryPoint x t) : (fillTypesInFunctions
        (lookVarInFunction xs (findWhichFunction x scopes))
    ) scopes
fillTypesInFunctions (x : xs) scopes = x : fillTypesInFunctions xs scopes

-- * ------------------------------------------ FILL TYPES USER FUNCTION ------------------------- * --

findWhichFunctionReturn :: String ->
    [(String, DataType, [(String, DataType, Int)])] -> DataType
findWhichFunctionReturn x [] =
    error ("Error: No function to remplace: " ++ show x)
findWhichFunctionReturn x ((name, t, _) : _) | name == x = t
findWhichFunctionReturn x (_ : ys) = findWhichFunctionReturn x ys

checkGoodTypeReturn :: [(String, DataType, Int)] -> String -> DataType ->
    DataType
checkGoodTypeReturn [] x t =
    error ("Error: No variable found: " ++ show x ++ " type: " ++ show t)
checkGoodTypeReturn ((name, t2, _) : _) x t | name == x && t2 == t = t
checkGoodTypeReturn (_ : ys) x t = checkGoodTypeReturn ys x t

remplaceNextStoreVar :: [Bytecode] -> [(String, DataType, Int)] -> DataType ->
    [Bytecode]
remplaceNextStoreVar [] _ _ = []
remplaceNextStoreVar (StoreVarBefore x _ : xs) scope t =
    (
        StoreVar (findId scope x) (checkGoodTypeReturn scope x t)
    ) : remplaceNextStoreVar xs scope t
remplaceNextStoreVar x _ _ = x

fillTypesUserFunction :: [Bytecode] -> String ->
    [(String, DataType, [(String, DataType, Int)])] -> [Bytecode]
fillTypesUserFunction [] _ _ = []
fillTypesUserFunction (FunEntryPoint x t : xs) _ scopes =
    (FunEntryPoint x t) : (fillTypesUserFunction xs x scopes)
fillTypesUserFunction (CallUserFun x : xs) name scopes =
    (CallUserFun x) : (
        fillTypesUserFunction (remplaceNextStoreVar xs
        (findWhichFunction name scopes)
        (findWhichFunctionReturn x scopes)) name scopes
    )
fillTypesUserFunction (x : xs) name scopes =
    x : fillTypesUserFunction xs name scopes
...
removeBeforeVariables (LoadVarBefore x _ : xs) v =
    (LoadVar (findId v x) (remplaceType v x)) : removeBeforeVariables xs v -- maybe remplaceType can be avoidable ?
removeBeforeVariables (StoreVarBefore x _ : xs) v =
    (StoreVar (findId v x) (remplaceType v x)) : removeBeforeVariables xs v -- maybe remplaceType can be avoidable ?
removeBeforeVariables (FunEntryPoint x t : xs) _ =
    (FunEntryPoint x t : xs) -- we find another scope, so we stop
removeBeforeVariables (x : xs) v =
    x : removeBeforeVariables xs v

fillLastTypes :: [Bytecode] ->
    [(String, DataType, [(String, DataType, Int)])] ->
        [Bytecode]
fillLastTypes [] _ = []
fillLastTypes (FunEntryPoint x t : xs) scopes =
    (FunEntryPoint x t) : (fillLastTypes (
        removeBeforeVariables xs (findWhichFunction x scopes)
    ) scopes)
...
saveStringAtTheEnd (LoadConst _ StringType : StringToSave str : xs) size =
    let (bcode, b) = saveStringAtTheEnd xs (size + 4 * ((length str) + 1)) in
    (LoadConst size StringType : bcode,  stringToBytes str ++ b)
saveStringAtTheEnd (x : xs) size =
    let (bcode, b) = saveStringAtTheEnd xs size in
    (x : bcode, b)

-- * ------------------------------------------ MAIN ---------------------------------------------- * --

bytecodeToBinary :: [Bytecode] -> String -> IO ()
bytecodeToBinary bytecode filename =
    let nmp_jmp = getNmbrOfJumps bytecode
        bytecode2 = remplaceAllJump bytecode 1 nmp_jmp
        (scopes_variables, _) = getScopesVariables bytecode2 0
        bytecode2_0 = fillTypesInFunctions bytecode2 scopes_variables
        bytecode2_1 = fillTypesUserFunction bytecode2_0 "" scopes_variables
        b2_2 = fillLastTypes bytecode2_1 scopes_variables
        bytecode3 = filter (\x -> case x of JumpRef _ -> False; _ -> True) b2_2
        funct_data = getFunctData bytecode3 sizeOfHeader
        b4 = remplaceAllFun bytecode3 funct_data
        posMain = findPosMain b4 sizeOfHeader
        (b4_1, b) = saveStringAtTheEnd b4 $ getSizeOfBytecode b4 + sizeOfHeader
        bytecode5 = Jump posMain :
            filter (\x -> case x of FunEntryPoint _ _ -> False; _ -> True) b4_1
        binary = getHeader ++ bytecodeToBytes bytecode5 ++ b
    in putStrLn "-- * ---------------------------- * --" >>
       ...
       print "binary" >>
       print binary >>
       when (posMain == 0) (error "Error: No main function found") >>
